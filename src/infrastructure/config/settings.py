...
def _parse_bool(name: str, default: bool) -> bool:
    raw_value = _read_env(name, str(default).lower()).lower()
    if raw_value in {"1", "true", "yes", "on"}:
        return True
    if raw_value in {"0", "false", "no", "off"}:
        return False
    raise SettingsError(
        f"Invalid boolean for '{name}': '{raw_value}'. Allowed: true/false, 1/0, yes/no, on/off."
    )
...
def load_settings() -> AppSettings:
    ...
    langchain_tracing_v2 = _parse_bool("LANGCHAIN_TRACING_V2", False)
    langchain_api_key = _read_env("LANGCHAIN_API_KEY", "")
    langchain_project = _read_env("LANGCHAIN_PROJECT", "atlas-rag-platform") or "atlas-rag-platform"
    langchain_endpoint = _read_env("LANGCHAIN_ENDPOINT", "https://api.smith.langchain.com")
    if not langchain_endpoint:
        langchain_endpoint = "https://api.smith.langchain.com"

    if langchain_tracing_v2 and not langchain_api_key:
        raise SettingsError(
            "Missing required environment variable 'LANGCHAIN_API_KEY' in stage 'phase-6-observability'. "
            "Set LANGCHAIN_API_KEY when LANGCHAIN_TRACING_V2=true."
        )
